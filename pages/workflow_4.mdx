...
> Workflow 1에서는 My Affirmations 앱을 테스트했지만, 이 앱은 확언 종류가 제한적입니다.
> Workflow 2에서는 Claude Instant 모델을 AWS 계정에 추가했습니다.
> Workflow 3에서는 생성형 AI를 사용해 새롭게 생성된 확언을 앱에 표시하도록 코드를 업데이트했습니다.
...
4. 지역 드롭다운 메뉴에서 **미국 서부 (오레곤) us-west-2**로 설정합니다.
    - **중요**: Amazon Bedrock 서비스는 해당 리전에서만 액세스할 수 있습니다.
...
8. **Enable specific models**를 선택하세요.
    - **Enable all models**를 선택하지 마세요. 이 경우 여러 권한 오류가 발생할 수 있습니다.
...
> **성공**: Amazon Bedrock 모델 ID를 확인하려면 [Amazon Bedrock 모델 ID 문서](https://docs.aws.amazon.com/bedrock/latest/userguide/model-ids.html)를 참조하세요.
